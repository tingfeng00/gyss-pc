...
// import { pathname } from '../service/config'
...
//   let origin = `${window.location.origin}/${pathname}/`
//   url = `${origin}${url}`
//   let appid = 'wxf24d72db02fede73'
//   let redirectUri = url && encodeURIComponent(url)
//   let responseType = 'code'
//   let scope = 'snsapi_base'
//   let wechatUrl = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${appid}&redirect_uri=${redirectUri}&response_type=${responseType}&scope=${scope}#wechat_redirect`
//   return wechatUrl
// }
/**
 * @desc 获取pc端页面地址
...
export const share = ({ url = '', title = '', source = '', description = '', summary = '', img = '', origin = '', type = '' }) => {
  const templates = {
    qzone: `https://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?to=qzone&url=${url}&desc=${description}&title=${title}&pic=${img}&site=${summary}`,
...
/**
 * @desc 使用jsencrypt方法加密
 */
export const rsaEnscrypt = (str, publicKey) => {
  if (!publicKey) {
  ...
  var strEncrypt = rsaProvider.encrypt(str.replace(/\+/g, '%2B'))
  return strEncrypt
}
